...
- MaaNTE 有三种控制器模式，定义在 `assets/interface.json`：
...
- **游戏分辨率**：必须 `1280x720` 窗口模式。新版钓鱼适配了任意 16:9 分辨率，但其他功能仍以 720p 为基准。
- **Windows 缩放**：必须 100%。非 100% 会导致点击坐标全部偏移（MaaFramework 限制）。
- **路径限制**：必须纯英文路径，不允许全角字符和特殊符号（MaaFramework 限制）。
...
- 买鱼饵误识别：鱼饵商店和方斯选项的 ROI 可能重叠。可建议降低 `鱼饵识别阈值`（默认 0.8，最低 0.6）。
- 卖鱼需要在 120 FPS 下可靠运行。
...
- "没有收益/没有全连击" → 需要娜娜莉和白藏的城市技能，是游戏机制不是 MaaNTE bug。

## Real-Time Assist

- 传送点仅支持"维特海默塔"和"ReroRero 电话亭"。
- "无法跳过重要剧情提示"是已知的功能缺失。
- 依赖 F1/F2/F5 快捷键，用户修改键位会导致功能失效。

## PinkPaw Heist

- 被标记为"极不稳定"，只有一个方案且需要特定队伍（娜娜莉、薄荷、早雾）。
- 容错超时后退出至主菜单是 by design。

## Log Collection Timing

- 日志文件在 MaaNTE 运行时被锁定，用户必须在关闭 MaaNTE 后收集。
- MaaNTE 每次启动会清理旧日志。如果用户复现后重启了 MaaNTE，关键日志可能已永久丢失。
