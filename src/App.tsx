...
function App() {
  ...
  useEffect(() => {
    ...
    document.addEventListener('maante-resize-start', onExternalResizeStart as EventListener);
    return () =>
      document.removeEventListener('maante-resize-start', onExternalResizeStart as EventListener);
  }, []);
  ...
  const loadInterface = async () => {
    ...
    try {
      ...
      // 加载用户配置（maante-{项目名}.json）- 从数据目录加载
      ...
      const dispatchPendingAutoStartTasks = () => {
        ...
        setTimeout(() => {
          ...
          new CustomEvent('maante-start-tasks', { detail: { source: 'autostart' } }),
          );
        }, 500);
      };
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (combo === startKey) {
        e.preventDefault();
        document.dispatchEvent(
          new CustomEvent('maante-start-tasks', { detail: { source: 'hotkey', combo } }),
        );
        return;
      }
      ...
      if (combo === stopKey) {
        e.preventDefault();
        document.dispatchEvent(
          new CustomEvent('maante-stop-tasks', { detail: { source: 'hotkey', combo } }),
        );
        return;
      }
    ...
    const registerKeys = async () => {
      try {
        ...
        await register(toTauriKey(startKey), () => {
          ...
          lastStartTime = now;
          document.dispatchEvent(
            new CustomEvent('maante-start-tasks', {
        ...
        if (stopKey !== startKey) {
          await register(toTauriKey(stopKey), () => {
            document.dispatchEvent(
              new CustomEvent('maante-stop-tasks', {
    ...
    const setupTrayListeners = async () => {
      try {
        ...
        unlistenStart = await listen('tray-start-tasks', () => {
          log.info('收到托盘开始任务事件');
          document.dispatchEvent(
            new CustomEvent('maante-start-tasks', { detail: { source: 'tray' } }),
          );
        });

        unlistenStop = await listen('tray-stop-tasks', () => {
          log.info('收到托盘停止任务事件');
          document.dispatchEvent(new CustomEvent('maante-stop-tasks', { detail: { source: 'tray' } }));
        });
    ...
    return (
      ...
      >
        ...
        <div className="relative z-10 h-full flex flex-col">
          ...
          Toolbar 内部监听 maante-start-tasks / maante-stop-tasks 并复用既有启动/停止逻辑。
  ...
  const getDisplayTitle = () => {
    if (!projectInterface) return { title: 'MaaNTE', subtitle: 'MaaFramework 下一代通用 GUI' };
