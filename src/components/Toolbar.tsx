...
const PRE_ACTION_CANCELLED_ERROR = 'MAANTE_PRE_ACTION_CANCELLED';
...
export function Toolbar({ showAddPanel, onToggleAddPanel, className }: ToolbarProps) {
  ...
  useEffect(() => {
    ...
    document.addEventListener('maante-start-tasks', handleStartTasks);
    document.addEventListener('maante-stop-tasks', handleStopTasks);

    return () => {
      document.removeEventListener('maante-start-tasks', handleStartTasks);
      document.removeEventListener('maante-stop-tasks', handleStopTasks);
    };
